...
export function InvoiceLineItemsEditor({
  ...
  const saveLines = useSaveInvoiceLineItems();
  ...
  const serverSignature = useMemo(
    () =>
      JSON.stringify({
        ...
        lines: invoice.line_items.map((line) => [line.id, line.updated_at]),
      }),
    [invoice],
  );
  ...
  return (
    <section className="rounded-xl border border-border bg-card">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-6 py-4">
        <div>
          <h2 className="text-[16px] font-semibold text-foreground">Invoice Items</h2>
              ...
              lines.map((line, index) => (
                <tr key={line.id ?? `draft-${index}`} className="border-b border-border/70">
                  ...
                  <td className="px-4 py-2 text-right align-top font-semibold text-foreground">
                    {formatMoney(totals.lines[index]?.line_total ?? 0, invoice.currency)}
                  </td>
                  {!readOnly ? (
                    <td className="px-3 py-2 align-top">
        ...
        <dl className="w-full max-w-sm space-y-2 text-sm">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <dd className="font-semibold text-foreground">
              −{formatMoney(totals.discount_amount, invoice.currency)}
            </dd>
          </div>

          <Row label="Tax" value={formatMoney(totals.tax_amount, invoice.currency)} />
